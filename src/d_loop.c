...
// Reduce the bandwidth needed by sampling game input less and transmitting less.
// If ticdup is 2, sample half normal, 3 = one third normal, etc.
int		ticdup;
...
static bool CanBuildNewTic() {
    ...
    if (new_sync) {
        ...
        // Never go more than ~250ms ahead
        return maketic - gameticdiv < 9;
    }
    // Never go more than ~150ms ahead
    return maketic - gameticdiv < 5;
}

static void BuildNewTic() {
    ticcmd_t cmd;
...
static int GetLowTic(void) {
    if (net_client_connected && (drone || recvtic < maketic)) {
        return recvtic;
    }
    return maketic;
}
...
static bool NetPlayersInGame() {
    if (!net_client_connected) {
        // Not connected to a server, so no players.
        return false;
    }
...
static bool PlayersInGame(void) {
    // Whether single or multiplayer, unless we are
    // running as a drone, we are in the game.
    if (drone) {
        return NetPlayersInGame();
    }
    return true;
}

//
// When using ticdup, certain values must be cleared out
// when running the duplicate ticcmds.
//
...
//
// When running in single player mode, clear all the
// ingame[] array except the local player.
//
...
static void UpdateGameState(int lowtic) {
...
//
static void RunGameSimulation(int counts) {
    int lowtic = GetLowTic();
    while (counts--) {
        if (!PlayersInGame()) {
            return;
        }
        UpdateGameState(lowtic);
...
// If there are no tics to run, then sleep until some are available.
//
static void WaitForNewTics(int entertic, int counts) {
    int lowtic = GetLowTic();
    ...
    while (!PlayersInGame() || lowtic < gametic/ticdup + counts) {
        ...
        if (lowtic < gametic/ticdup) {
            I_Error("WaitForNewTics: lowtic < gametic");
        }
...
//
static int CountNumTics(int realtics) {
    int lowtic = GetLowTic();
...
void TryRunTics(void) {
    ...
    int counts = CountNumTics(realtics);
    WaitForNewTics(entertic, counts);
    RunGameSimulation(counts);
}
