...
static bool F_CanShowArtScreen() {
    if (gamemode == commercial) {
        // no art screen for Doom II.
        return false;
    }
    if (finalestage != F_STAGE_TEXT) {
        // only show art screen after text.
        return false;
    }
    // only show art screen after finish displaying text.
    return finalecount > strlen(finaletext) * TEXTSPEED + TEXTWAIT;
}

static void F_AdvanceAnimation() {
    finalecount++;
}

static void F_SkipFinale() {
    if (gamemap == 30) {
        F_StartCast();
        return;
    }
...
static bool F_CanSKipFinale() {
    if (gamemode != commercial) {
        // skipping only allowed in Doom II.
        return false;
    }
    if (finalecount <= 50) {
        // give some time to show the finale screen.
        return false;
    }
    // go on to the next level if player pressed button
...
static void F_DrawPatchCol(int x, patch_t *patch, int col) {
    ...
    while (column->topdelta != END_COLUMN) {
        for (int row = 0; row < column->length; row++) {
            int screen_y = row + column->topdelta;
            int screen_spot = x + (screen_y * SCREENWIDTH);
            I_VideoBuffer[screen_spot] = column->data[row];
        }
