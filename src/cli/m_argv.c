...
static bool FileIsDemoLump(const char *filename)
{
    FILE *handle;
    int count, ver;
    byte buf[12], *p = buf;
...
static int GuessFileType(const char *name)
{
    ...
    if (!iwad_found && D_IsIWADName(lower)) {
        ret = FILETYPE_IWAD;
        iwad_found = true;
        ...
        if (!demo_found && FileIsDemoLump(name)) {
            ret = FILETYPE_DEMO;
            demo_found = true;
        } else {
            ret = FILETYPE_PWAD;
