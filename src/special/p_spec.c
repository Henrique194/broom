...
side_t* getSide(int currentSector, int line, int side) {
    const sector_t* curr_sec = &sectors[currentSector];
    short side_num = curr_sec->lines[line]->sidenum[side];
...
sector_t* getSector(int currentSector, int line, int side) {
    side_t* side_ptr = getSide(currentSector, line, side);
    return side_ptr->sector;
}


//
// twoSided()
// Given the sector number and the line number, it will tell
// you whether the line is two-sided or not.
//
...
// Array containing the heights of sectors with a height strictly
// greater than "currentheight".
static fixed_t heightlist[MAX_ADJOINING_SECTORS + 2];

static int P_FindLowestHeight(int currentheight, int adjoined_secs) {
    if (adjoined_secs == 0) {
...
static int P_AdjoinSectors(const sector_t *sec, int currentheight) {
    int adjoined_secs = 0;
    fixed_t height = currentheight;

    for (int i = 0; i < sec->linecount; i++) {
        line_t* check = sec->lines[i];
        const sector_t* other = getNextSector(check, sec);
        if (other == NULL) {
            continue;
        }
        ...
        if (adjoined_secs == MAX_ADJOINING_SECTORS + 1) {
            height = other->floorheight;
        } else if (adjoined_secs == MAX_ADJOINING_SECTORS + 2) {
            // Fatal overflow: game crashes at 22 sectors
            I_Error("Sector with more than 22 adjoining sectors. "
                    "Vanilla will crash here");
        }

        heightlist[adjoined_secs] = other->floorheight;
        adjoined_secs++;
    }

    return adjoined_secs;
}
...
fixed_t P_FindNextHighestFloor(const sector_t* sec, int currentheight) {
    int adjoined_secs = P_AdjoinSectors(sec, currentheight);
    return P_FindLowestHeight(currentheight, adjoined_secs);
}
...
int P_FindMinSurroundingLight(const sector_t* sector, int max) {
    ...
    for (int i = 0; i < sector->linecount; i++) {
        ...
        const sector_t* check = getNextSector(line, sector);
        if (check == NULL) {
            continue;
        }
...
//
static bool P_CanTriggerSpecialEvent(const mobj_t* thing, const line_t* line) {
    if (gameversion <= exe_doom_1_2 && line->special > 98
        && line->special != 104)
    {
        return false;
    }
...
void P_CrossSpecialLine(int linenum, int side, mobj_t* thing) {
    ...
    if (!P_CanTriggerSpecialEvent(thing, line)) {
        return;
    }
...
static bool P_CanShootSpecialLine(const mobj_t* thing, const line_t* line) {
    if (thing->player) {
        return true;
    }
    // Impacts that other things can activate.
    // OPEN DOOR IMPACT
    return line->special == 46;
}
...
void P_ShootSpecialLine(mobj_t* thing, line_t* line) {
    if (!P_CanShootSpecialLine(thing, line)) {
        return;
    }
    switch (line->special) {
        case 24:
...
static void P_DoE1M8FinaleDamage(player_t* player) {
...
static void P_PlayerHitSecretSector(player_t* player) {
    sector_t* sector = player->mo->subsector->sector;
    sector->special = 0;    // not a secret anymore
    player->secretcount++;  // intermission stats
}

static void P_DoStrobeDamage(player_t* player) {
    if ((player->powers[pw_ironfeet] && P_Random() >= 5) || (leveltime & 0x1f)) {
        return;
    }
    P_DamageMobj(player->mo, NULL, NULL, 20);
}

static void P_DoSuperHellSlimeDamage(player_t* player) {
    P_DoStrobeDamage(player);
}

static void P_DoNukageDamage(player_t* player) {
...
static void P_DoHellSlimeDamage(player_t* player) {
    if (player->powers[pw_ironfeet] || (leveltime & 0x1f)) {
        return;
    }
...
void P_PlayerInSpecialSector(player_t* player) {
    const sector_t* sector = player->mo->subsector->sector;
    ...
    switch (sector->special) {
        case 4:
            ...
            break;
        case 5:
            P_DoHellSlimeDamage(player);
            break;
        case 7:
            P_DoNukageDamage(player);
            break;
        case 9:
            P_PlayerHitSecretSector(player);
            break;
        case 11:
            P_DoE1M8FinaleDamage(player);
            break;
        case 16:
            P_DoSuperHellSlimeDamage(player);
            break;
        default:
...
static void P_AnimateButton(button_t* button) {
    short button_texture = (short) button->btexture;
    side_t* side = &sides[button->line->sidenum[0]];
    switch (button->where) {
        case top:
            side->toptexture = button_texture;
            break;
        case middle:
            side->midtexture = button_texture;
            break;
        case bottom:
            side->bottomtexture = button_texture;
            break;
    }
...
static void P_AnimateButtons() {
    for (int i = 0; i < MAXBUTTONS; i++) {
        ...
        P_AnimateButton(button);
    }
}

static void P_AnimateLineSpecials() {
...
//
static void P_AnimateTexturesAndFlats() {
    for (const anim_t* anim = anims; anim < lastanim; anim++) {
        int first_anim = anim->basepic;
        int last_anim = first_anim + anim->numpics - 1;

        for (int i = first_anim; i <= last_anim; i++) {
            int anim_offset = (leveltime / anim->speed + i) % anim->numpics;
            int pic = first_anim + anim_offset;
            if (anim->istexture) {
...
void P_UpdateSpecials() {
    ...
    P_AnimateTexturesAndFlats();
    P_AnimateLineSpecials();
    P_AnimateButtons();
}
...
static void P_InitPlats() {
    for (int i = 0; i < MAXPLATS; i++) {
        activeplats[i] = NULL;
    }
}

static void P_InitCeilings() {
    for (int i = 0; i < MAXCEILINGS; i++) {
        activeceilings[i] = NULL;
    }
}

static void P_InitLineEffects() {
    numlinespecials = 0;
...
static void P_SpawnSpecialSector(sector_t* sector) {
    switch (sector->special) {
        case 1:
            // FLICKERING LIGHTS
...
static void P_InitSpecialSectors() {
    for (int i = 0; i < numsectors; i++) {
        sector_t* sector = &sectors[i];
        if (sector->special) {
            P_SpawnSpecialSector(sector);
        }
    }
}

static void P_InitLevelTimer() {
