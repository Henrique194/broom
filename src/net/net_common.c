...
bool NET_Conn_IsDisconnected(const net_connection_t* con) {
    net_connstate_t con_state = con->state;
    return con_state == NET_CONN_STATE_DISCONNECTED
           || con_state == NET_CONN_STATE_DISCONNECTED_SLEEP;
}

static bool NET_Conn_IsDisconnecting(const net_connection_t* con) {
    return con->state == NET_CONN_STATE_DISCONNECTING;
}

void NET_Conn_Disconnect(net_connection_t *conn) {
    if (NET_Conn_IsDisconnected(conn) || NET_Conn_IsDisconnecting(conn)) {
        return;
    }
...
//
static void NET_CONN_RunDisconnectedSleep(net_connection_t *conn, unsigned int nowtime) {
...
//
static void NET_CONN_RunDisconnecting(net_connection_t *conn, unsigned int nowtime) {
    net_packet_t *packet;
...
static void NET_CONN_RunConnected(net_connection_t *conn, unsigned int nowtime) {
    net_packet_t *packet;
...
void NET_Conn_Run(net_connection_t *conn) {
    ...
    switch (conn->state) {
        case NET_CONN_STATE_CONNECTED:
            NET_CONN_RunConnected(conn, nowtime);
            break;
        case NET_CONN_STATE_DISCONNECTING:
            NET_CONN_RunDisconnecting(conn, nowtime);
            break;
        case NET_CONN_STATE_DISCONNECTED_SLEEP:
            NET_CONN_RunDisconnectedSleep(conn, nowtime);
            break;
        default:
            break;
    }
