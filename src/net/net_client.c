...
static void NET_CL_ParseSYN(net_packet_t *packet) {
    ...
    char *server_version = NET_ReadSafeString(packet);
    if (server_version == NULL) {
        NET_Log("client: error: failed to read server version");
        return;
    }

    net_protocol_t protocol = NET_ReadProtocol(packet);
    ...
    // Even though we have negotiated a compatible protocol, the game may
    // still desync. Broom's philosophy makes this unlikely, but if we're
    // playing with a forked version, or even against a different version
    // that fixes a compatibility issue, we may still have problems.
...
static void NET_CL_ParseReject(net_packet_t *packet) {
    const char *msg = NET_ReadSafeString(packet);
    if (msg == NULL) {
        return;
    }
...
static void NET_CL_ParseGameData(net_packet_t *packet) {
    ...
    // Whatever happens, we now need to send an acknowledgement
    // of our current receive point.
...
static void NET_CL_ParseConsoleMessage(net_packet_t *packet) {
    char *msg = NET_ReadSafeString(packet);
    if (msg) {
...
static void NET_CL_UpdateWaitingStatus() {
    net_waiting_for_launch =
        client_connection.state == NET_CONN_STATE_CONNECTED
        && client_state == CLIENT_STATE_WAITING_LAUNCH;
}

static void NET_CL_ReceiveServerPackets() {
    net_addr_t *addr;
    net_packet_t *packet;
...
void NET_CL_Run(void) {
    ...
    NET_CL_ReceiveServerPackets();
    // Run the common connection code to send any packets as needed
    NET_Conn_Run(&client_connection);
    if (NET_Conn_IsDisconnected(&client_connection)) {
        NET_CL_Disconnected();
        NET_CL_Shutdown();
    }
    NET_CL_UpdateWaitingStatus();
...
static void NET_CL_TryConnect(net_addr_t *addr, net_connect_data_t *data) {
    int start_time = I_GetTimeMS();
    int last_send_time = -1;
    bool sent_hole_punch = false;
...
void NET_CL_Disconnect(void) {
    ...
    while (!NET_Conn_IsDisconnected(&client_connection)) {
