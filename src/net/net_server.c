...
//
// Returns true if we have tics from all players for first tic in the
// recv window. Returns false if the first tic is not complete: ie. we have
// not received tics from all connected players. This can happen if only one
// player is in the game.
//
static bool NET_SV_ReceivedTicFromAllPlayers() {
...
//
static void NET_SV_TryAdvanceWindow(void) {
    ...
    while (recvwindow_start < lowtic) {
        if (!NET_SV_ReceivedTicFromAllPlayers()) {
            break;
        }
...
static void NET_SV_CheckResendsConnectedPlayers() {
    for (int i = 0; i < NET_MAXPLAYERS; ++i) {
        if (sv_players[i] && ClientConnected(sv_players[i])) {
            NET_SV_CheckResends(sv_players[i]);
        }
    }
}

static void NET_SV_RunState() {
    switch (server_state) {
        ...
        case SERVER_WAITING_START:
            ...
            break;
        case SERVER_IN_GAME:
            NET_SV_TryAdvanceWindow();
            NET_SV_CheckResendsConnectedPlayers();
            break;
    }
}

//
// "Run" any clients that may have things to do, independent of
// responses to received packets
//
static void NET_SV_RunActiveClients() {
    for (int i = 0; i < MAXNETNODES; ++i) {
        if (clients[i].active) {
...
static void NET_SV_ReceivePackets() {
    net_addr_t *addr;
    net_packet_t *packet;
...
void NET_SV_Run(void) {
    ...
    NET_SV_ReceivePackets();
    if (master_server) {
        UpdateMasterServer();
    }
    NET_SV_RunActiveClients();
    NET_SV_RunState();
}
