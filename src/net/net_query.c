...
static query_target_t* NewTargetForAddr(net_addr_t *addr) {
...
static query_target_t* FindTargetForAddr(net_addr_t *addr) {
...
static query_target_t* GetTargetForAddr(net_addr_t *addr, bool create) {
    query_target_t *target = FindTargetForAddr(addr);
    if (target) {
        return target;
    }
    if (create) {
        return NewTargetForAddr(addr);
    }
    return NULL;
}
...
//
// Found a target to query.
// Send a query; how to do this depends on the target type.
//
static void SendQueryForTarget(query_target_t* target, unsigned int now) {
    switch (target->type) {
...
static query_target_t* FindTargetForQuery(unsigned int now) {
    for (int i = 0; i < num_targets; ++i) {
        // Not queried yet?
        // Or last query timed out without a response?
        query_target_t* target = &targets[i];
        query_target_state_t state = target->state;
        unsigned int diff_time = now - target->query_time;

        if (state == QUERY_TARGET_QUEUED
            || (state == QUERY_TARGET_QUERIED && diff_time > QUERY_TIMEOUT_SECS * 1000))
        {
            return target;
        }
    }
...
static void SendOneQuery(void) {
    ...
    query_target_t* target = FindTargetForQuery(now);
    if (target) {
        SendQueryForTarget(target, now);
    }
}
...
static bool AllTargetsDone(void) {
    for (int i = 0; i < num_targets; ++i) {
        query_target_state_t state = targets[i].state;
        if (state != QUERY_TARGET_RESPONDED && state != QUERY_TARGET_NO_RESPONSE) {
            return false;
        }
    }
    return true;
