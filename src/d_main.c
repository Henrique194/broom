...
//
// D-DoomLoop()
// Not a globally visible function, just included for source reference, called
// by D_DoomMain, never exits. Manages timing and IO, calls all ?_Responder,
// ?_Ticker, and ?_Drawer, calls I_GetTime, I_StartFrame, and I_StartTic
//
void D_DoomLoop();
...
static void D_DrawPausePic() {
    int x = viewwindowx + (scaledviewwidth - 68) / 2;
    int y = automapactive ? 4 : viewwindowy + 4;

    const char* lump_name = DEH_String("M_PAUSE");
    patch_t* patch = (patch_t *) W_CacheLumpName(lump_name, PU_CACHE);
...
// Variables used for doing screen melt effect
static int wipestart = 0;
static bool wipe = false;

static void D_DoWipe() {
    // Runs at least 1 tic.
    int tics;
    int nowtime;
    do {
...
static void D_CheckIncompatibleIwad() {
    if (gamevariant != bfgedition) {
        return;
    }
    if (demorecording || gameaction == ga_playdemo || netgame) {
...
static void D_DoHelpScreen() {
    gamestate = GS_DEMOSCREEN;
    pagetic = 200;
    pagename = DEH_String("HELP2");
}

static void D_DoCreditScreen() {
    gamestate = GS_DEMOSCREEN;
    pagetic = 200;
    pagename = DEH_String("CREDIT");
}

static void D_DoTitleScreen() {
    gamestate = GS_DEMOSCREEN;

    // The Doom 3: BFG Edition version of doom2.wad does not have a
    // TITLETPIC lump. Use INTERPIC instead as a workaround.
    if (gamevariant == bfgedition && W_CheckNumForName("titlepic") < 0) {
...
static void D_RunDemoSequence() {
    switch (demosequence) {
        ...
        case 6:
            ...
            break;
        default:
            I_Error("D_RunDemoSequence: invalid demo sequence %d", demosequence);
    }
}

static void D_UpdateDemoSequence() {
    // The Ultimate Doom executable changed the demo sequence to add
    // a DEMO4 demo. Final Doom was based on Ultimate, so also includes
    // this change; however, the Final Doom IWADs do not include a DEMO4
    // lump, so the game bombs out with an error when it reaches this point
    // in the demo sequence.
    //
...
//
// We can manually override the gamemission that we got from the IWAD detection
// code. This allows us to eg. play Plutonia 2 with Freedoom and get the right
// level names.
//
static void D_TryOverrideGameMission() {
    if (gamemode != commercial) {
        return;
    }
    //!
...
static void D_IdentifyGameMode() {
...
//
// gamemission is set up by the D_FindIWAD function. But if we specify '-iwad',
// we have to identify using IdentifyIWADByName. However, if the iwad does not
// match any known IWAD name, we may have a dilemma. Try to identify by its
// contents.
//
static void D_IdentifyGameMission() {
    if (gamemission != none) {
        // already set up by the D_FindIWAD.
        return;
    }
...
static void D_IdentifyVersion() {
    D_IdentifyGameMission();
    D_IdentifyGameMode();
    D_TryOverrideGameMission();
}
...
static int GetDemoVersion(int demo) {
    char demolumpname[6];
    M_snprintf(demolumpname, 6, "demo%i", demo);

    if (W_CheckNumForName(demolumpname) <= 0) {
        return -1;
    }
    const byte* demolump = W_CacheLumpName(demolumpname, PU_STATIC);
    int demo_version = demolump[0];
...
//
static GameVersion_t DetectVersionFromDemoLump() {
    for (int demo = 1; demo <= 3; demo++) {
        int demo_version = GetDemoVersion(demo);
        if (demo_version < 0) {
            continue;
        }
        switch (demo_version) {
...
static void IdentifyGameVersion() {
    switch (gamemode) {
        case shareware:
        case registered:
            gameversion = DetectVersionFromDemoLump();
            break;
        case retail:
...
static void EmulateDoomVersion(const char* selected_version) {
    for (int i = 0; gameversions[i].description != NULL; i++) {
        if (!strcmp(selected_version, gameversions[i].cmdline)) {
    ...
    I_Error("Unknown game version '%s'", selected_version);
}
...
static void InitGameVersion() {
    ...
    if (p) {
        const char* selected_version = myargv[p + 1];
        EmulateDoomVersion(selected_version);
    } else {
...
static void LoadFrenchIwadDeh() {
    // Look for french.deh in the same directory as the IWAD file.
    char* dirname = M_DirName(iwadfile);
    char* french_deh = M_StringJoin(dirname, DIR_SEPARATOR_S, "french.deh", NULL);
    printf("French version\n");
    free(dirname);

    // If the dehacked patch isn't found, try searching
    // the WAD search path instead. We might find it...
...
static void LoadChexIwadDeh() {
    // Look for chex.deh in the same directory as the IWAD file.
    char* dirname = M_DirName(iwadfile);
    char* chex_deh = M_StringJoin(dirname, DIR_SEPARATOR_S, "chex.deh", NULL);
    free(dirname);
...
static bool IsFrenchIWAD() {
    return gamemission == doom2
           && W_CheckNumForName("M_RDTHIS") < 0
           && W_CheckNumForName("M_EPISOD") < 0
           && W_CheckNumForName("M_EPI1") < 0
           && W_CheckNumForName("M_EPI2") < 0
           && W_CheckNumForName("M_EPI3") < 0
           && W_CheckNumForName("WIOSTF") < 0
           && W_CheckNumForName("WIOBJ") >= 0;
}
...
void D_DoomMain() {
    ...
    M_SetConfigFilenames("default.cfg", PACKAGE_TARNAME ".cfg");
    D_BindVariables();
    M_LoadDefaults();
