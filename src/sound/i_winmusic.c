...
static bool IsPartLevel(const byte *msg, unsigned int length)
{
    if (length == 10 &&
        msg[0] == 0x41 && // Roland
        msg[2] == 0x42 && // GS
...
static bool IsSysExReset(const byte *msg, unsigned int length)
{
    if (length < 5)
    {
        return false;
...
static bool AddToBuffer_Vanilla(unsigned int delta_time,
...
static bool AddToBuffer_Standard(unsigned int delta_time,
                                    const midi_event_t *event,
                                    win_midi_track_t *track)
{
...
static bool (*AddToBuffer)(unsigned int delta_time,
                              const midi_event_t *event,
...
static bool I_WIN_InitMusic(void)
{
    int all_devices;
    int i;
    MIDIOUTCAPS mcaps;
...
static void I_WIN_PlaySong(void *handle, bool looping)
{
    MMRESULT mmr;
...
static bool I_WIN_MusicIsPlaying(void)
{
