...
static bool I_FL_InitMusic(void)
{
    int sf_id;
...
static void I_FL_PlaySong(void *handle, bool looping)
{
    if (player)
    {
...
static bool IsMid(const byte *mem, int len)
{
...
static bool I_FL_MusicIsPlaying(void)
{
    if (player == NULL)
    {
        return false;
