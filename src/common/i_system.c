...
static void I_ShowErrorDialogBox(const char* msgbuf) {
    Uint32 flags = SDL_MESSAGEBOX_ERROR;
    const char* title = PACKAGE_STRING;
    SDL_Window* window = NULL;
    SDL_ShowSimpleMessageBox(flags, title, msgbuf, window);
}
...
//
static bool I_CanShowErrorDialogBox() {
    //!
    // @category obscure
    //
    ...
    return exit_gui_popup && !I_ConsoleStdout();
}

static void I_Shutdown() {
...
void I_Error(const char *error, ...) {
    ...
    if (I_CanShowErrorDialogBox()) {
        I_ShowErrorDialogBox(msgbuf);
    }
