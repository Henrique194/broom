...
static void saveg_read_ticcmd_t(ticcmd_t* str) {
    ...
    str->forwardmove = (char) saveg_read8();

    // signed char sidemove;
    str->sidemove = (char) saveg_read8();
...
void P_UnArchivePlayers() {
    for (int i = 0; i < MAXPLAYERS; i++) {
        ...
        // Will be set when unarchive thinker.
        players[i].mo = NULL;
...
static void P_UnArchiveSectors() {
...
//
// Clear thinkers data before loading new mobjs.
//
static void P_ResetThinkers() {
    // Remove all the current thinkers.
    thinker_t* curr_thinker = thinkercap.next;
    while (curr_thinker != &thinkercap) {
        thinker_t* next = curr_thinker->next;
        if (curr_thinker->function.acp1 == (actionf_p1) P_MobjThinker) {
            P_RemoveMobj((mobj_t*) curr_thinker);
        } else {
            Z_Free(curr_thinker);
        }
        curr_thinker = next;
...
static void P_UnArchiveGlowLight() {
    saveg_read_pad();
...
static void P_UnArchiveStrobeLight() {
    saveg_read_pad();
...
static void P_UnArchiveFlashLight() {
    saveg_read_pad();
...
static void P_UnArchivePlatform() {
    saveg_read_pad();
...
static void P_UnArchiveCeiling() {
    saveg_read_pad();
    ...
    while (true) {
        ...
        switch (tclass) {
            ...
            case tc_plat:
                ...
                break;
            case tc_flash:
                P_UnArchiveFlashLight();
                break;
            case tc_strobe:
                P_UnArchiveStrobeLight();
                break;
            case tc_glow:
                P_UnArchiveGlowLight();
                break;
            case tc_endspecials:
                // End of list.
                return;
