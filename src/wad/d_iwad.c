...
static char *CheckDirectoryHasIWAD(const char *dir, const char *iwadname)
{
    ...
    if (probe != NULL && DirIsFile(dir, iwadname)) {
        return probe;
    }
...
static bool D_CanUseIwad(const iwad_t* iwad, int mask) {
    return ((1 << iwad->mission) & mask) != 0;
}
                                    ...
                                    GameMission_t *mission)
{
    for (size_t i = 0; i < arrlen(iwads); ++i) {
        const iwad_t* iwad = &iwads[i];
        if (!D_CanUseIwad(iwad, mask)) {
            continue;
        }

        const char* iwad_name = DEH_String(iwad->name);
        char* filename = CheckDirectoryHasIWAD(dir, iwad_name);
        if (filename != NULL) {
...
static GameMission_t IdentifyIWADByName(const char* name, int mask) {
    ...
    for (size_t i = 0; i < arrlen(iwads); i++) {
        const iwad_t* iwad = &iwads[i];
        if (D_CanUseIwad(iwad, mask) && !strcasecmp(name, iwad->name)) {
            return iwad->mission;
        }
    }
...
static void BuildDefaultIwadDirs() {
    // Look in the current directory.
    // Doom always does this.
    AddIWADDir(".");
    ...
    const char* env = M_getenv("DOOMWADDIR");
    if (env != NULL) {
        AddIWADDir(env);
    }
...
char* D_FindWADByName(const char* name) {
    ...
    for (int i = 0; i < num_iwad_dirs; ++i) {
        ...
        if (probe != NULL && DirIsFile(iwad_dirs[i], name)) {
            return probe;
        }
        free(probe);
...
//
static char* D_SearchIwads(int mask, GameMission_t* mission) {
    BuildIWADDirList();

    for (int i = 0; i < num_iwad_dirs; ++i) {
        char* result = SearchDirectoryForIWAD(iwad_dirs[i], mask, mission);
        if (result != NULL) {
            return result;
        }
    }
...
//
static char* D_SearchIwadFile(int iwadparm, int mask, GameMission_t* mission) {
    const char* iwadfile = myargv[iwadparm + 1];
    char* result = D_FindWADByName(iwadfile);
    if (result == NULL) {
...
// registered/commercial features should be executed (notably loading PWADs).
//
char* D_FindIWAD(int mask, GameMission_t* mission) {
    ...
    if (iwadparm) {
        return D_SearchIwadFile(iwadparm, mask, mission);
    }
    return D_SearchIwads(mask, mission);
}
