...
static SRC_ERROR linear_vari_process(SRC_STATE* state, SRC_DATA* data) {
    ...
    while (input_index < 1.0 && priv->out_gen < priv->out_count) {
        if (priv->in_used + state->channels * (1.0 + input_index) >=
            priv->in_count)
            break;
        ...
        for (ch = 0; ch < state->channels; ch++)
        {
            data->data_out[priv->out_gen] =
                (float) (priv->last_value[ch] +
                         input_index * ((double) data->data_in[ch] -
                                        priv->last_value[ch]));
            priv->out_gen++;
        }
           ...
           priv->in_used + state->channels * input_index < priv->in_count)
    {
        ...
        {
            printf("Whoops!!!!   in_used : %ld     channels : %d     "
                   "input_index : %f\n",
                ...
                (float) (data->data_in[priv->in_used - state->channels + ch] +
                         input_index *
                             ((double) data->data_in[priv->in_used + ch] -
                              data->data_in[priv->in_used - state->channels +
                                            ch]));
            priv->out_gen++;
        }
...
static SRC_STATE* linear_copy(SRC_STATE* state) {
    ...
    const LINEAR_DATA* from_priv = (LINEAR_DATA *) state->private_data;
    ...
    memcpy(to_priv->last_value, from_priv->last_value,
           sizeof(float) * state->channels);
