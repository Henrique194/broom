...
// Sanity Check
typedef int _CHECK_SHIFT_BITS[2 * (SHIFT_BITS < sizeof(increment_t) * 8 - 1) - 1];
...
typedef struct
{
    ...
    double left_calc[MAX_CHANNELS];
    double right_calc[MAX_CHANNELS];
...
static SRC_ERROR prepare_data(SINC_FILTER* filter, int channels, const SRC_DATA* data, int half_filter_chan_len) WARN_UNUSED;
...
static inline int int_div_ceil(int divident, int divisor) {
    // For positive numbers only
    assert(divident >= 0 && divisor > 0);
    // == (int) ceil ((float) divident / divisor)
...
static SINC_FILTER* sinc_filter_new(int converter_type, int channels) {
    ...
    switch (converter_type) {
        ...
        case SRC_SINC_MEDIUM_QUALITY:
            ...
            priv->coeff_half_len =
                ARRAY_LEN(slow_mid_qual_coeffs.coeffs) - 2;
        ...
        case SRC_SINC_BEST_QUALITY:
            ...
            priv->coeff_half_len =
                ARRAY_LEN(slow_high_qual_coeffs.coeffs) - 2;
    ...
    priv->b_len = 3 * psf_lrint((priv->coeff_half_len + 2.0) / priv->index_inc * SRC_MAX_RATIO + 1);
    priv->b_len = MAX(priv->b_len, 4096);
    priv->b_len *= channels;
    // There is a <= check against samples_in_hand requiring a buffer
    // bigger than the calculation above
    priv->b_len += 1;
...
static SRC_STATE* sinc_copy(SRC_STATE* state) {
    ...
    const SINC_FILTER* from_filter = (SINC_FILTER *) state->private_data;
...
static inline double calc_output_single(const SINC_FILTER* filter,
                                        ...
                                        increment_t start_filter_index)
{
    ...
    if (data_index < 0) {
        ...
        // If the assert triggers we would have to take
        // care not to underflow/overflow
...
static SRC_ERROR sinc_mono_vari_process(SRC_STATE* state, SRC_DATA* data) {
    ...
    while (filter->out_gen < filter->out_count) {
        ...
        data->data_out[filter->out_gen] = (float) ((float_increment / filter->index_inc) * calc_output_single(filter, increment, start_filter_index));
        filter->out_gen++;
...
static inline void calc_output_stereo(const SINC_FILTER* filter, int channels,
                                      increment_t increment,
                                      increment_t start_filter_index,
...
static SRC_ERROR sinc_stereo_vari_process(SRC_STATE* state, SRC_DATA* data) {
    ...
    while (filter->out_gen < filter->out_count) {
        ...
        calc_output_stereo(filter, state->channels, increment,
                           start_filter_index,
...
static inline void calc_output_quad(const SINC_FILTER* filter, int channels,
                                    increment_t increment,
                                    increment_t start_filter_index,
                                    double scale, float* output)
{
...
static inline void calc_output_hex(const SINC_FILTER* filter, int channels,
                                   increment_t increment,
...
static SRC_ERROR sinc_multichan_vari_process(SRC_STATE* state, SRC_DATA* data) {
    ...
    while (filter->out_gen < filter->out_count) {
        ...
        calc_output_multi(filter, increment, start_filter_index,
                          state->channels, float_increment / filter->index_inc,
                          data->data_out + filter->out_gen);
...
static SRC_ERROR prepare_data(SINC_FILTER* filter, int channels,
                              const SRC_DATA* data, int half_filter_chan_len)
{
    int len = 0;

    if (filter->b_real_end >= 0) {
        // Should be terminating. Just return.
        return SRC_ERR_NO_ERROR;
    }
    ...
    if (filter->in_used == filter->in_count && filter->b_end - filter->b_current < 2 * half_filter_chan_len && data->end_of_input) {
        // Handle the case where all data in the current buffer
        // has been consumed and this is the last buffer.
